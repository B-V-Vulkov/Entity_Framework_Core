    ...
    public class StartUp
    {
        public static void Main()
        {
            using (SqlConnection connection = new SqlConnection(Configuration.CoonnectionString))
            {
                ...
                string DBNane = "[MinionsDB]";
                string createDB = $"CREATE DATABASE {DBNane}";
                string useDB = $"USE {DBNane}";

                ExecuteNonQuery(connection, createDB);
                ExecuteNonQuery(connection, useDB);
                ...
                foreach (var statemen in createStatements)
                {
                    ExecuteNonQuery(connection, statemen);
                }
                ...
                foreach (var statemen in insertStatements)
                {
                    ExecuteNonQuery(connection, statemen);
                }
            }
